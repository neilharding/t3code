/**
 * DevinDriver — `ProviderDriver` for the Devin CLI (`devin acp`) runtime.
 *
 * Devin exposes an ACP-based CLI, similar to Cursor and Grok. Model
 * catalog refreshes happen during the managed provider status check via
 * Devin's `model` session config option (see `DevinProvider.ts`).
 *
 * Text generation is supported via the ACP runtime — `makeDevinTextGeneration`
