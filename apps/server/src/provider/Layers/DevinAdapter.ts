 ...
 *
 * Devin's ACP dialect is close to Cursor's: session config options carry
 * the model catalog, and permission requests use the standard
 * `allow_once` / `allow_always` / `reject_once` option kinds. Unlike
 * Cursor, Devin emits Cognition-specific extension requests and
 * notifications (`_cognition.ai/*`) that this adapter does not otherwise
 * understand — unhandled ones would tear the ACP session down, so this
 * adapter registers fallback handlers that no-op them instead.
 *
...
const ACP_BYPASS_MODE_ALIASES = ["bypass"];
const ACP_APPROVAL_MODE_ALIASES = ["ask"];
const ACP_IMPLEMENT_MODE_ALIASES = ["accept-edits", "code", "default", "agent"];
...
export interface DevinAdapterLiveOptions {
   ...
   * value. Defaults to the legacy built-in instance id (`devin`).
   */
...
/**
 * Maps T3's runtime/interaction modes onto Devin's four ACP session
 * modes (`accept-edits`, `ask`, `plan`, `bypass`). `approval-required`
 * maps onto `ask` even though Devin's `accept-edits` mode still applies
 * file edits automatically — command approvals still route through
 * `session/request_permission` in either mode, which is the best match
 * available today.
 */
...
/**
 * Auto-approves a permission request when T3's runtime mode grants Devin
 * blanket approval (`full-access` / `auto` / `auto-accept-edits`).
 * `approval-required` and `plan` always surface the request to the user.
 */
function shouldAutoApprovePermissions(runtimeMode: RuntimeMode): boolean {
  return (
    runtimeMode === "full-access" || runtimeMode === "auto" || runtimeMode === "auto-accept-edits"
  );
}
    ...
    const startSession: DevinAdapterShape["startSession"] = (input) =>
      withThreadLock(
        ...
        Effect.gen(function* () {
          ...
          // Devin cannot take T3's per-thread HTTP MCP server today —
          // `devin acp` advertises `mcpCapabilities.http/sse: false` — so
          // unlike Cursor and Grok this adapter never forwards an MCP
          // server. Documented in docs/user/providers-devin.md.
          ...
          const started = yield* Effect.gen(function* () {
            // Devin emits Cognition-specific extension requests/notifications
            // (e.g. `_cognition.ai/output`) that this adapter does not model.
            // Without fallback handlers, an unhandled extension request tears
            // the ACP session down.
            yield* acp.handleUnknownExtRequest(() => Effect.succeed(null));
            yield* acp.handleUnknownExtNotification((method, params) =>
              logNative(input.threadId, method, params).pipe(Effect.orDie),
            );
            yield* acp.handleRequestPermission((params) =>
              mapExtensionFailure(
                Effect.gen(function* () {
                  yield* logNative(input.threadId, "session/request_permission", params);
                  if (shouldAutoApprovePermissions(input.runtimeMode)) {
